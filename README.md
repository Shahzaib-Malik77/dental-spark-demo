...
PRD — Dental Care Clinic Demo Landing Page

Product: Single-Page Dental Clinic Website (Client Demo/Lead-Gen Template) Prepared for: Zaphtra Ltd — Client Presentations & Lead Generation Version: 1.0

1. Overview (Roman Urdu)

Ye ek premium single-page dental clinic landing page ka demo hai jo Zaphtra Ltd apne potential clients (dental clinics, dentists, healthcare practices) ko dikhane ke liye use karega. Maqsad ye hai ke client ko live, polished, animated website dikha kar unhe convince kiya jaye ke wo humse apni clinic ki website banwayein.

Ye ek universal, reusable demo hai — matlab isko kisi bhi dental clinic ke naam/branding ke sath customize karke pitch kiya ja sakta hai.

2. Goal / Business Purpose

Ek visually impressive demo banana jo client meetings, WhatsApp, portfolio, ya cold outreach me share ho sake.

Client ko dikhana ke Zaphtra modern, animated, mobile-responsive websites bana sakta hai.

Lead generation tool ke tor par use karna — demo link bhej kar interest generate karna.

3. Target Audience

User Type Description Primary Dental clinics / individual dentists jo apni online presence upgrade karna chahte hain Secondary Zaphtra ke apne clients jo reference dekhna chahte hain ke quality kaisi hai

4. Tech Stack

Framework: React + Vite + TypeScript

Styling: Tailwind CSS (no external UI/icon libraries)

Structure: Single App.tsx file (lightweight, fast demo build)

Font: "Open Sauce One" (custom loaded via link tags)

Hosting suggestion: Vercel/Netlify free tier ya Render (jaisa demo site pehle use ho chuka hai)

5. Page Structure (High-Level)

Splash Screen — 0–100 loading counter animation (2 seconds), professional first impression

Fixed Navbar — Logo + desktop menu button + mobile hamburger menu

Section 1 — Hero — "Dental Care" heading, trust statement, free consultation CTA

Section 2 — Smile Gallery — Services showcase (Veneers, Crowns, Whitening, Implants) + Call CTA

Section 3 — Implant Dentistry — Detailed service education + booking CTA

6. Core Design Concept — "Masked Cards"

Sections 1 & 2 ek unique technique use karte hain jise "Masked Cards" kehte hain:

Ek hi bara background image multiple cards ke through "windows" ki tarah dikhaya jata hai.

Har card us image ka alag portion show karta hai, jis se cohesive, magazine-style mosaic look banta hai.

Ye technique responsive hai (mobile/desktop dono par alag focal points).

Client ko pitch karte waqt highlight karein: "Ye ek custom-engineered visual technique hai jo generic templates me nahi milti — ye premium/agency-level design signal deta hai."

7. Key Features Checklist

[ ] Splash screen with animated counter

[ ] Sticky glass-effect navbar (blur background)

[ ] Mobile slide-in menu with staggered link animations

[ ] Masked-card mosaic hero section

[ ] Scroll-triggered reveal animations (fade + slide up) on every section

[ ] Services grid with active/inactive card states

[ ] Fully responsive (single breakpoint: 768px — mobile vs desktop)

[ ] Clean black/white/glass color palette (professional, medical, trustworthy)

[ ] CTA buttons: "Call Us", "Book Online", "Book Appointment"

8. Design System

Element Rule Colors Strictly black, white, translucent white (glassmorphism) Typography Bold/Extrabold, tight leading, large clamp() headings Corners Rounded-xl/2xl on all cards Motion Smooth cubic-bezier transitions, staggered reveals Buttons Scale-up hover effect (105%)

9. Animations

Animation Behavior Splash counter 0→100 in 2000ms, then fade out Section reveal Cards fade + slide up as user scrolls (IntersectionObserver) Mobile menu Slide-in panel + staggered link entrance Buttons Hover scale transform

10. Deliverable / Output

Ek live, deployable demo link jo browser me kaam kare (desktop + mobile dono).

Source code clean aur reusable — future me kisi bhi clinic ke naam/images/colors se rebrand ho sake.

11. Success Criteria (Lead-Gen Angle)

Demo dekh kar client ko lage ke ye agency-quality, custom-built website hai (template jaisa na lage).

Loading/animation smooth ho, koi lag ya broken layout na ho mobile par.

Client easily samajh sake ke apni clinic ke liye kya customize hoga (naam, images, services, colors).

12. Suggested Pitch Line (for outreach)

"Ye ek demo hai jo humne dental clinics ke liye design kiya — animated, mobile-friendly, aur modern. Agar aapki clinic ki website is level ki nahi hai, hum 1-2 hafton me exactly aisi bana sakte hain."

Difficult Words / Mushkil Alfaaz (Glossary)

Word Simple Meaning (Roman Urdu) PRD Product Requirements Document — ek document jo batata hai product me kya hona chahiye Landing Page Ek single web page jo pehla impression dene ke liye banaya jata hai Glassmorphism Design style jisme cheezein sheeshay/frosted glass jaisi transparent dikhti hain IntersectionObserver Ek browser feature jo detect karta hai ke element screen par kab dikhna shuru hua Responsive Website jo mobile aur desktop dono par sahi tarah adjust ho jaye CTA (Call To Action) Wo button/text jo user ko kisi action (call, book) ke liye ubharta hai Cubic-bezier Ek math curve jo animation ko smooth/natural banata hai Reusable Template Aisa design jo alag-alag clients ke liye dobara use ho sake 

Design prompt (Create a single-page dental clinic landing page using **React + Vite + TypeScript + Tailwind CSS**. No external UI libraries, no icon libraries. Everything lives in one `App.tsx` file. The page has 3 full-screen sections, a splash screen, and a fixed navbar.
...
`, `h-screen w-full overflow-hidden flex flex-col`, `pt-24 md:pt-24 px-3 md:px-5 pb-1.5 md:pb-2 gap-1.5 md:gap-2`
...
- Content: `` centered vertically and horizontally (`flex items-center justify-center h-full`), `text-black text-lg md:text-3xl font-bold text-center`, `relative z-10`
...
- **Top-left text:** `absolute top-4 left-4 md:top-7 md:left-7`, `text-black text-xs md:text-sm font-semibold leading-4 md:leading-5 max-w-[200px] md:max-w-[300px] z-10`
  - Content: "We wish to provide professional dental services" `
...
` with `text-black text-[clamp(3rem,11vw,11rem)] font-bold leading-[0.79] tracking-tight`, content: "Dental" `
` "Care"
...
`, `min-h-screen md:h-screen w-full overflow-hidden flex flex-col`, `pt-1.5 md:pt-2 px-3 md:px-5 pb-1.5 md:pb-2 gap-1.5 md:gap-2`
...
- Text: `absolute bottom-16 left-5 md:bottom-20 md:left-7`, `text-white text-xs md:text-sm font-semibold leading-4 md:leading-5 z-10`, content: "If you want a gorgeous smile," `
...
- Heading: `absolute top-4 left-5 md:top-6 md:left-7`, `text-white md:text-black text-[clamp(3rem,7vw,6rem)] font-bold leading-[0.9] z-10`, content: "Smile" `
` "makeover"
...
` with `text-xl md:text-4xl font-bold leading-[1.05] whitespace-pre-line`, color: active=`text-black`, inactive=`text-white`
...
Does NOT use MaskedCard technique. Uses regular `` tags and solid backgrounds.
...
`: `rounded-xl md:rounded-2xl bg-stone-50 p-5 md:p-7 flex flex-col justify-between flex-[1.2] min-h-[180px] md:min-h-0`
- Animated: `s3Reveal.getAnimStyle(0)`
- Heading: `

` with `text-[clamp(3rem,7vw,6.5rem)] font-bold leading-[0.95] text-black`, content: "Implant" `
` "Dentistry"
- Subtitle: `

` with `text-xs md:text-sm font-semibold text-black`, text "Restore Missing Teeth"

**2. Two Image Cards (side by side):**
- Wrapper: `

` with `flex gap-1.5 md:gap-2 flex-1 min-h-[140px] md:min-h-0`
...
`: `rounded-xl md:rounded-2xl bg-zinc-200 p-5 md:p-7 flex items-end justify-between flex-[0.8] min-h-[160px] md:min-h-0`
- Animated: `s3Reveal.getAnimStyle(2)`
- Left content block:
  - Label: `

` with `text-xs md:text-sm font-semibold text-black mb-2 md:mb-3`, text "Consultation"
  - Heading: `

` with `text-xl md:text-3xl font-bold text-black leading-6 md:leading-8`, content: "Dental" `
` "Restoration" `
` "Services"
...
`: `rounded-xl md:rounded-2xl overflow-hidden relative min-h-[350px] md:min-h-0`
...
` with `text-lg md:text-2xl font-bold text-black leading-5 md:leading-7`, content: "The Process" `
` "of Installing" `
` "Implants"
...
` with `text-lg md:text-2xl font-bold text-white leading-5 md:leading-7`, content: "Caring" `
` "for Dental" `
` "Implants"
...
` containing:
1. `{showSplash && }` (conditionally rendered)
2. ``
3. Section 1
4. Section 2
5. Section 3
...
Continue developing this project in the [Lovable editor](https://lovable.dev/projects/d68f7ba1-8865-4615-8ffb-5cdfcbad5591).
